...
Évidemment, les œuvres et les artistes précédemment cités, le sont non seulement à titre d'exemples, d'illustrations, mais aussi, sont particulièrement significatives et significatifs dans leur radicalité et leur chronologie, pour y être effectivement mentionnés.
...
\noindent \textbf{Nous entendons tous, mais nous n'écoutons pas les même choses.}

\bigskip

%\textit{La recherche d’une typologie « absolue » est illusoire.} Cependant, 
Loin de prétendre à une description absolue et exhaustive, je me contenterai d'énoncer quelques pistes, usant de la terminologie d'un compositeur, sans que cela soit inscrit dans le marbre. En tout état de cause, il est possible de décrire un objet sonore selon des caractéristiques pertinentes que l'on souhaite formaliser ou formuler dans le but de disposer d'un langage commun au groupe impliqué.
\bigskip

De l'objet sonore:

\begin{longtable}{rcccl}
durée courte & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & durée longue \\
\textit{quantum} & $\leftarrow$ & grain & $\rightarrow$ & lisse \\
régulier & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & chaotique \\
continu & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & discontinu \\
son pur & $\leftarrow$ & son cannelé & $\rightarrow$ & bruit blanc \\
monophonie & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & polyphonie \\
hauteurs définis & $\leftarrow$ & masse fixe & $\rightarrow$ & masse variable \\
%grave & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & aigu \\
\textit{rallentendo} & $\leftarrow$ & \textit{a tempo} & $\rightarrow$ & \textit{accelerando} \\
hyper-localisation & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & ubiquité \\
près & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & loin \\
petit espace clos & $\leftarrow$ & -- -- -- -- -- -- & $\rightarrow$ & grand espace clos \\
...
variation,  articulation,  punctuation or  transition
